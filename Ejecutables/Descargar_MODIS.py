...
year    = '2014'
tiles   = ['h11v12','h11v11']
package  = r'D:\SystemData\Desktop\NASA_DEVELOP_SRM'
...
def Fetch_MODIS(product, version, tiles, years, outdir, Quiet=False):
    ...
    try:
        if ftp:
            dates=List_ftp_contents(site,Dir)
            print(dates)
...
def Find_MODIS_Product(product,version):
    ...
    if sat_designation=='MOD':
        if ftp: Dir = 'DP4/MOST/'+product+'.'+version
        else:   site = site1+'MOLT/'+product+'.'+version
    # if sat_designation=='MOD':
    #     if ftp: Dir = 'MOST/'+product+'.'+version
    #     else:   site = site1+'MOLT/'+product+'.'+version
...
#===================================================================================
print('Start')
Fetch_MODIS('MOD10A1','061',tiles,year,os.path.join(package,r'Datos\NASA_Datos\MODIS'),False)
